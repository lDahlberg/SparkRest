...
public class ApiTest {
    ...
    public void addingReviewToUnknownCourseThrowsError() throws Exception {
        ...
        ApiResponse res = client.request("POST", "/courses/32/reviews", gson.toJson(values));
        assertEquals(500, res.getStatus());
    }

    @Test
    public void reviewsCanBeAccessedFromCourseId() throws Exception {
        Course course = newTestCourse();
        courseDao.add(course);
        Review review = new Review(course.getId(), 5, "Great");
        Review review2 = new Review(course.getId(), 5, "Great");
